...
jQuery.fn.smartFilter = function (opts) {
    var $this = jQuery(this);

    var log = function (msg, type) {
        if (typeof console == "object") console.log("[smartFilter - " + (type || "Erro") + "] " + msg);
    };
    ...
    var defaults = {
        ...
        filterScrollTop: function (shelfOffset) {
            let headerHeight = jQuery(document).width() > 980 ? 90 : 50;
            return shelfOffset - headerHeight;
        },
        callback: function () {},
    ...
    // Colocando no window para que seja compartilhado entre os smartFilters (para todos olharem para o mesmo urlFilter)    
    window.urlFilters = "";

    var fn = {
        getUrl: function (scroll) {
            var s = scroll || false;
            if (s) return currentSearchUrl.replace(/PageNumber=[0-9]*/, "PageNumber=" + currentPage);
            else return (searchUrl + window.urlFilters).replace(/PageNumber=[0-9]*/, "PageNumber=" + pageNumber);
        },
        ...
        nextPage: function (currentStatus) {
            if (!lockRequest) {
                ...
                pageJqxhr = jQuery.ajax({
                    ...
                    success: function (data) {
                        if (data.trim().length < 1) {
                            ...
                            const $data = $(data)[1];
                            $data.setAttribute("data-page", currentPage - 1);
                        ...
                        for (const $helper of document.getElementsByClassName("helperComplement")) {
                            $helper.remove();
                        }
                    },
                });
                ...
                // ScrollBack
                var time = window.localStorage.getItem("productId") != null ? 1 : 500;
        ...
        infinitScroll: function () {
            ...
            // ScrollBack
            if (!iOS()) {
                ScrollBack(() => fn.nextPage(currentStatus));
            }
    ...
    var fns = {
        exec: function () {
            ...
            $this.each(function () {
                ...
                if (_this.is(":checked")) {
                    window.urlFilters += "&" + (_this.attr("rel") || "");
            ...
            if ("" !== window.urlFilters) fns.addFilter($empty);
        },
        mergeMenu: function () {
        ...
        fieldsetFormat: function () {
            ...
            filtersMenuE.find("fieldset").each(function () {
                var $t = jQuery(this),
                    ...
                    fieldsetClass =
                        "filtro_" +
                        ($t.find("h5:first").text() || "").toLowerCase().replaceSpecialChars().replace(/\s/g, "-");
        ...
        addFilter: function (input) {
            // Quebra o rel por virgula
            const inputFilters = input.attr("rel").split(',');

            lastInputFilter = input;

            const splitedUrlFilters = window.urlFilters.split('&');

            // Percorre a lista de fqs do rel
            inputFilters.forEach(filter => {
                // Verifica se já tem esse fq na url e adiciona caso não tenha
                if (!splitedUrlFilters.includes(filter)) window.urlFilters += "&" + (filter || "");
            });

            window.lastFilter = input.attr("rel") || "";
        ...
        removeFilter: function (input) {
            ...
            let newUrlFilters = window.urlFilters.split('&');
            
            // Percorre a lista de fqs do rel
            inputFilters.forEach(filter => {
                // Verifica se não é vazio para remover
                if (filter !== "") newUrlFilters = newUrlFilters.filter(fq => fq !== filter);
            });

            window.urlFilters = newUrlFilters.join('&');
        ...
        filterAjaxSuccess: function (data) {
            ...
            // cálculo customizado de acordo com a necessidade da loja
            let pageToolsOffsetTop = jQuery("section.pagetools").length
                ? jQuery("section.pagetools").offset().top
                : loadContentE.offset().top;
            _html.animate({ scrollTop: options.filterScrollTop(pageToolsOffsetTop) }, 600);
        },
        ...
        updateContent: function ($data) {
            ...
            (shelfPage.length > 0 ? shelfPage : options.emptySearchElem).slideUp(600, function () {
                ...
                if (shelf.length > 0) {
                    ...
                    // fns.removeFilter(lastInputFilter);
        ...
        adjustText: function (input) {
            ...
            // Verifica se tem img, caso tenha não executa, pois isso remove a img
            if (!label[0].getElementsByTagName('img').length) label.text(text).prepend(input);
        },
